...
public class GameMap {
    ...
    public GameMap() {
        ...
        for (int i = 0; i < CELLS_X; i++) {
            for (int j = 0; j < CELLS_Y; j++) {
                ...
                this.data[i][j].moveCost = MathUtils.random(1,2);
            }
        }
    ...
    public void render(SpriteBatch batch) {
        for (int i = 0; i < CELLS_X; i++) {
            for (int j = CELLS_Y - 1; j >= 0; j--) {
                batch.setColor(1.0f, 1 - data[i][j].moveCost / 5f, 1.0f, 1.0f );
    ...
    public int getMoveCost(int cellX, int cellY) {
        return data[cellX][cellY].moveCost;
    }
}
